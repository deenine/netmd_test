...
def print_hex(direction, message):
    if direction == 'out':
        print "out -> " + " ".join("{:02x}".format(x) for x in message)
    elif direction == 'in':
        print "in <- " + " ".join("{:02x}".format(ord(c)) for c in message)
    else:
        print message

def to_hex_str(direction, message):
    if direction == 'out':
        return "out -> " + " ".join("{:02x}".format(x) for x in message)
    elif direction == 'in':
        return "in <- " + " ".join("{:02x}".format(ord(c)) for c in message)
    else:
        print message
...
def iterdevices(usb_context, bus=None, device_address=None, jfdi=False):
    """
...
class NetMDInterface(object):
    ...
    def send_query(self, query, test=False):
        ...
        if DEBUG: print_hex('out', query)
        status, reply = self.readReply(query)
        if DEBUG:
            print_hex('in', reply)
            print "%02x" % status
        if status == STATUS_INTERIM:
            if DEBUG: print 're-reading'
            status, reply = self.readReply(query)
            if DEBUG:
                print_hex('in', reply)
                print STATUS[status]
    ...
    def readReply(self, query):
        ...
        status = ord(result[0])
        output = to_hex_str('out', query) + '\n'
        output += to_hex_str('in', result[1:]) + '\n'
        output += 'netmd status: %02X\n' % status
        if status == STATUS_NOT_IMPLEMENTED:
            if DEBUG: print_hex('in', result)
    ...
    def getStatus(self):
        ...
        if HANDSHAKE:
            # send handshake
            if DEBUG: print('handshake: on')
            query = self.formatQuery('1808 8000 0100')
            reply = self.send_query(query)
            self.scanQuery(reply, '1808 8000 0100')
        query = self.formatQuery('1809 8001 0230 8800 0030 8804 00 ff00 ' \
                                 '00000000')
        reply = self.send_query(query)
        #1000 00090000 00 07 88 04 00 03 40 ff 40
    ...
    def isDiskPresent(self):
        ...
        if int("".join("{:02x}".format(ord(c)) for c in status[4])) == 40:
            return True
        else:
            return False
    ...
    def getOperatingStatus(self):
        if HANDSHAKE:
            query = self.formatQuery('18 08 80 00 01 00')
            reply = self.send_query(query)
    ...
    def getTrackCount(self):
        ...
        """
        if HANDSHAKE:
            query = self.formatQuery('18 08 10 10 01 01 00')
            reply = self.send_query(query)
...
#py         18 07 02 20 18 01 00 00 30 00 0a 00 50 00 00 08 00 00 00 04 00 09 54 65 73 74 20 37 38 30 00
    ...
    def eraseTrack(self, track):
        ...
        reply = self.send_query(query)
        self.scanQuery(reply, '1840 0001 00 201001 %?%?')
    ...
    def moveTrack(self, source, dest):
        ...
        """
        if HANDSHAKE:
            query = self.formatQuery('18 08 10 10 01 00 00')
            reply = self.send_query(query)

        #  py     move 0,1 -    18 43 ff 00 00 20 10 01 00 00 01 20 10 01 00 00 03
        # netmdcl move 1,2 - 00 18 43 ff 00 00 20 10 01 00 01 20 10 01 00 02
        query = self.formatQuery('1843 ff00 00 201001 %w 201001 %w', source,
                                 dest)
        reply = self.send_query(query)
                               #1843 0000 00 201001 00 01   201001 00 03
    ...
    def getDiscCapacity(self):
        ...
        """
        if HANDSHAKE:
            query = self.formatQuery('18 08 10 10 00 01 00')
            reply = self.send_query(query)
